            ...
            if player1.gesture == key:
                if cpu.gesture in gestures_dict[key] or player2.gesture in gestures_dict[key]:
                    ...
                    if mode_selected == 'Single-Player':
                        user_interface.sp_round_winner(player1)
                        return
                    else:
                        user_interface.mp_round_winner(player1, player2, True)

                else:
                    Score.cpu_and_player2_scores(self, mode_selected, player1, player2, cpu)
                    return

    def cpu_and_player2_scores(self, mode_selected, player1, player2, cpu):
        if mode_selected == 'Single-Player':
            self.cpu_wins += 1
            user_interface.sp_round_winner(cpu)
        elif mode_selected == 'Multi-Player':
            self.player2_wins += 1
            user_interface.mp_round_winner(player1, player2, False)
